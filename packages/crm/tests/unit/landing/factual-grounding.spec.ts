...
import { serviceSlug } from "../../../src/lib/landing/r1-site-tree";
...
test("sanitizes an existing DesertCool payload conservatively", () => {
  ...
  assert.equal(sanitized.hero.primaryCTA.href, "/intake");
  assert.equal(
  ...
  assert.equal(sanitized.servicePages?.length, 4);
  assert.deepEqual(
    sanitized.servicePages?.map((page) => page.slug),
    sanitized.services.services.map((service) => serviceSlug(service.name)),
  );
  assert.ok(sanitized.servicePages?.every((page) => page.name !== "AC Repair and Maintenance"));
});

test("synchronizes grounded service pages and preserves exact matches", () => {
  const incoming = payload();
  incoming.services.services = [
    { id: "s1", name: "AC Repair and Maintenance", description: "Grounded AC service description." },
    ...incoming.services.services.slice(1),
  ];
  incoming.servicePages = [
    {
      slug: "stale-slug",
      name: "AC repair",
      summary: "Detailed AC repair summary.",
      body: [{ kind: "heading", text: "Detailed AC repair process" }],
      gallery: [{ src: "/ac.jpg", alt: "AC repair" }],
      ctaLabel: "Contact us",
    },
    {
      slug: "ac-repair-and-maintenance",
      name: "AC Repair and Maintenance",
      summary: "Rich combined page content.",
      body: [{ kind: "paragraph", text: "Rich combined page body." }],
      ctaLabel: "Book service",
    },
  ];

  const sanitized = sanitizeR1LandingPayload(incoming, facts);
  assert.deepEqual(sanitized.services.services.map((service) => service.name), facts.services);
  assert.equal(sanitized.servicePages?.length, facts.services.length);
  assert.deepEqual(sanitized.servicePages?.map((page) => page.slug), facts.services.map(serviceSlug));
  assert.ok(!sanitized.servicePages?.some((page) => page.name === "AC Repair and Maintenance"));
  assert.equal(sanitized.servicePages?.[0]?.summary, "Detailed AC repair summary.");
  assert.equal(sanitized.servicePages?.[0]?.body[0]?.text, "Detailed AC repair process");
  assert.equal(sanitized.servicePages?.[1]?.summary, "AC repair, AC installation, HVAC maintenance, Emergency HVAC service.");
  assert.equal(sanitized.servicePages?.[1]?.body[0]?.text, "AC repair, AC installation, HVAC maintenance, Emergency HVAC service.");
  assert.equal(sanitized.servicePages?.[1]?.ctaLabel, "Book service");
  assert.ok(sanitized.servicePages?.every((page, index) => page.slug === serviceSlug(sanitized.services.services[index]!.name)));
});

test("grounded page fallbacks use the sanitized service and safe CTA", () => {
  const incoming = payload();
  incoming.services.services = [];
  const sanitized = sanitizeR1LandingPayload(incoming, facts);

  assert.equal(sanitized.servicePages?.length, 4);
  assert.equal(sanitized.servicePages?.[0]?.summary, "DesertCool HVAC provides AC repair.");
  assert.deepEqual(sanitized.servicePages?.[0]?.body, [{ kind: "paragraph", text: "DesertCool HVAC provides AC repair." }]);
  assert.ok(sanitized.servicePages?.every((page) => page.ctaLabel === "Book service"));
  assert.ok(sanitized.servicePages?.every((page) => !/free estimate/i.test(page.ctaLabel)));
});

test("without grounded services, existing service pages are preserved and scrubbed", () => {
  const noServicesFacts = buildVerifiedBusinessFacts({ business_name: "Plain Business" });
  const incoming = payload();
  incoming.servicePages = [{
    slug: "old-service",
    name: "Old Service",
    summary: "Licensed and insured old service.",
    body: [{ kind: "paragraph", text: "Free estimate available." }],
    ctaLabel: "Get a free estimate",
  }];

  const sanitized = sanitizeR1LandingPayload(incoming, noServicesFacts);
  assert.equal(sanitized.servicePages?.length, 1);
  assert.equal(sanitized.servicePages?.[0]?.slug, "old-service");
  assert.doesNotMatch(JSON.stringify(sanitized.servicePages), /licensed|insured|free estimate/i);
});
