...
import { sendExistingUserMagicLinkAction } from "./actions";
...
export function LoginForm({ redirectTo }: { redirectTo?: string | null }) {
  ...
  const [state, action, pending] = useActionState(sendExistingUserMagicLinkAction, {});
      ...
      {state.sent ? (
        <div className="space-y-3 rounded-xl border border-border bg-card p-4">
          <p className="text-sm text-foreground">
            If this email is registered, a sign-in link will arrive shortly.
          </p>
        </div>
      ) : null}
