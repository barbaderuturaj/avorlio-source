...
import { buildVerifiedBusinessFacts, sanitizePublicClaimText, sanitizeR1LandingPayload } from "@/lib/landing/factual-grounding";
...
export default async function WorkspaceLandingPage({ params }: PageProps) {
  ...
  const verifiedFacts = buildVerifiedBusinessFacts(ctx.soul, ctx.settings);
  ...
  const groundedPayload = sanitizeR1LandingPayload(r1.payload, verifiedFacts);
  const payload = rewriteR1Hrefs(groundedPayload, {
    book: bookingUrl,
    intake: intakeUrl,
