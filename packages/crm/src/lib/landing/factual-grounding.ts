...
const CLAIM_PATTERNS: Array<{ pattern: RegExp; supported: (facts: VerifiedBusinessFacts) => boolean }> = [
  ...
  { pattern: /\bsame(?:[-\u2010-\u2015\u2212]|\s)day\b/i, supported: (facts) => facts.sameDay },
...
export function sanitizePublicHtml(html: string, facts: VerifiedBusinessFacts): string {
  ...
  return html
    ...
    .replace(/<[^>]+>[^<>]*(?:5[- ]star|ratings?|reviews?|testimonials?|licensed|insured|financing|same(?:[-\u2010-\u2015\u2212]|\s)day|free\s+(?:estimate|quote)|24\s*\/\s*7)[^<>]*<\/[^>]+>/gi, "")
    .replace(/\b(?:5[- ]star rated by local customers|licensed and insured|same(?:[-\u2010-\u2015\u2212]|\s)day service available|free estimates?|free quotes?|financing available)\b/gi, "")
    .replace(/\s{2,}/g, " ");
}
...
export function sanitizeR1LandingPayload(
  ...
  if (p.servicePages) {
    const pages = facts.services.length
      ? p.servicePages.filter((page) => facts.services.some((service) => service.toLowerCase() === page.name.toLowerCase()))
      : p.servicePages;
    p.servicePages = pages.map((page) => ({
      ...page,
      name: facts.services.find((service) => service.toLowerCase() === page.name.toLowerCase()) ?? page.name,
...
export function sanitizeChatbotEmbedUrl(
  ...
  try {
    ...
    const publicAgentEmbed =
      stored.pathname.startsWith("/api/v1/public/agent/") &&
      stored.pathname.endsWith("/embed.js");
    if (localRequest || localStored || (preferRequestOrigin && publicAgentEmbed)) {
