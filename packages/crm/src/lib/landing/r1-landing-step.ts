...
export function withDefaultLeadForm(
  ...
  payload.leadForm = {
    ...
    heading: existing.heading ?? "Request service",
    subheading:
      existing.subheading ??
...
export async function runR1LandingStep(args: {
  ...
  try {
    ...
    let payload = await generateR1Payload({ facts, archetype, byokKey });
    ...
    // Final public factual-safety gate. Runs after service-page generation so
    // the hero, services, FAQs, lead form, footer, and detail pages are all
    // grounded from verified workspace facts before anything is persisted.
    payload = sanitizeR1LandingPayload(
      payload,
      buildVerifiedBusinessFacts(facts),
    );
