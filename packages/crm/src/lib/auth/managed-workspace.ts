...
type ManagedWorkspaceAccessInput = {
  workspace: {
    ownerId: string | null;
...
function hasManagedWorkspaceAccess(
  input: ManagedWorkspaceAccessInput,
): boolean {
  if (
    input.workspace.ownerId === input.userId ||
    input.workspace.parentUserId === input.userId
  ) {
    return true;
  }
  ...
  return Boolean(
    input.agency.ownerWorkspaceId &&
      input.agencyOwnerWorkspaceOwnerId === input.userId,
  );
}
