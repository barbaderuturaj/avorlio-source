...
apply_compat_migrations() {
  echo "[docker-migrate] applying additive compatibility migrations..."
  psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f ./drizzle/0023_scheduled_triggers.sql
}

if [ "$(psql "$DATABASE_URL" -tAc "SELECT to_regclass('public.organizations') IS NOT NULL")" = "t" ]; then
  echo "[docker-migrate] schema already present."
  apply_compat_migrations
  echo "[docker-migrate] compatibility migrations complete."
  exit 0
fi
